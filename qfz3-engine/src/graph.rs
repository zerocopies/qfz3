...
impl fmt::Display for ForwardError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ComputeFailed(status) => write!(f, "compute failed with status {status}"),
...
impl LlamaHparams {
    pub fn from_model(model: &MappedModel) -> Self {
        ...
        let get_u32 =
            |key: &str| -> i64 { meta.get(key).and_then(|v| v.as_u32()).unwrap_or(0) as i64 };
        let get_f32 = |key: &str, default: f32| -> f32 {
            meta.get(key)
                .and_then(|v| {
                    if let crate::gguf::GgufValue::F32(f) = v {
                        Some(*f)
                    } else {
                        None
                    }
        ...
        LlamaHparams {
            ...
            n_rot: {
                let r = get_u32(&format!("{}.rope.dimension_count", arch));
                if r > 0 {
                    r
                } else {
...
impl KVCache {
    ...
    pub fn new(
        ...
        for i in 0..n_layers {
            unsafe {
                ffi::ggml_backend_tensor_set(k[i], zeros.as_ptr() as *const c_void, 0, bytes_per);
                ffi::ggml_backend_tensor_set(v[i], zeros.as_ptr() as *const c_void, 0, bytes_per);
            }
        }
...
impl LlamaGraph {
    ...
    fn build_decode_graph(&mut self, model: &MappedModel) -> Result<()> {
        ...
        if inp_buf.is_null() {
            bail!("[Z.1 Decode] input buffer alloc failed");
        }
        ...
        unsafe {
            ffi::ggml_backend_tensor_alloc(inp_buf, d_token, inp_base as *mut c_void);
        }

        let d_pos = unsafe { ffi::ggml_new_tensor_1d(inp_ctx, 26, n_tokens) };
        unsafe {
            ffi::ggml_backend_tensor_alloc(inp_buf, d_pos, (inp_base + 64) as *mut c_void);
        }

        // mask [n_ctx, 1] f32 — added to kq inside soft_max_ext
        let d_mask = unsafe { ffi::ggml_new_tensor_2d(inp_ctx, 0, n_ctx, 1) };
        unsafe {
            ffi::ggml_backend_tensor_alloc(inp_buf, d_mask, (inp_base + 128) as *mut c_void);
        }

        // Initialise mask: all positions invalid
        let neg_inf = vec![-10000.0f32; n_ctx as usize];
        unsafe {
            ffi::ggml_backend_tensor_set(d_mask, neg_inf.as_ptr() as *const c_void, 0, mask_bytes);
        }
        ...
        for layer in 0..hp.n_layer as usize {
            ...
            let normed = unsafe {
                ffi::ggml_mul(
                    ctx,
                    ffi::ggml_rms_norm(ctx, cur, hp.rms_norm_eps),
                    t("attn_norm.weight")?,
                )
            };
            ...
            let q = match model.layer_tensor(layer, "attn_q.bias") {
                Some(b) => unsafe { ffi::ggml_add(ctx, q, b) },
                None => q,
            };
            let k = match model.layer_tensor(layer, "attn_k.bias") {
                Some(b) => unsafe { ffi::ggml_add(ctx, k, b) },
                None => k,
            };
            let v = match model.layer_tensor(layer, "attn_v.bias") {
                Some(b) => unsafe { ffi::ggml_add(ctx, v, b) },
                None => v,
            };
            ...
            let k_flat = unsafe {
                ffi::ggml_reshape_2d(ctx, ffi::ggml_cont(ctx, k), hp.n_head_kv * hd, n_tokens)
            };
            let v_flat = unsafe {
                ffi::ggml_reshape_2d(ctx, ffi::ggml_cont(ctx, v), hp.n_head_kv * hd, n_tokens)
            };
            unsafe {
            ...
            let k_full = unsafe {
                ffi::ggml_view_2d(ctx, k_cache, hp.n_head_kv * hd, n_ctx, kv_row_stride, 0)
            };
            let v_full = unsafe {
                ffi::ggml_view_2d(ctx, v_cache, hp.n_head_kv * hd, n_ctx, kv_row_stride, 0)
            };

            let k_3d = unsafe {
                ffi::ggml_reshape_3d(ctx, ffi::ggml_cont(ctx, k_full), hd, hp.n_head_kv, n_ctx)
            };
            let v_3d = unsafe {
                ffi::ggml_reshape_3d(ctx, ffi::ggml_cont(ctx, v_full), hd, hp.n_head_kv, n_ctx)
            };

            // Attention — mask applied inside soft_max_ext
            let scale = 1.0 / (hd as f32).sqrt();
            let q_perm = unsafe { ffi::ggml_permute(ctx, q, 0, 2, 1, 3) };
            let k_perm = unsafe { ffi::ggml_cont(ctx, ffi::ggml_permute(ctx, k_3d, 0, 2, 1, 3)) };
            let v_perm = unsafe { ffi::ggml_cont(ctx, ffi::ggml_permute(ctx, v_3d, 1, 2, 0, 3)) };

            let kq = unsafe { ffi::ggml_scale(ctx, ffi::ggml_mul_mat(ctx, k_perm, q_perm), scale) };
            ...
            let gate = unsafe {
                ffi::ggml_silu(ctx, ffi::ggml_mul_mat(ctx, t("ffn_gate.weight")?, ffn_in))
            };
            let up = unsafe { ffi::ggml_mul_mat(ctx, t("ffn_up.weight")?, ffn_in) };
            let ffn = unsafe {
                ffi::ggml_mul_mat(ctx, t("ffn_down.weight")?, ffi::ggml_mul(ctx, gate, up))
            };
        ...
        cur =
            unsafe { ffi::ggml_mul(ctx, ffi::ggml_rms_norm(ctx, cur, hp.rms_norm_eps), out_norm) };
        let lm_head = model
        ...
        log::info!(
            "[Z.1 Decode] persistent decode graph built (n_ctx={}, layers={})",
            n_ctx,
            hp.n_layer
        );
        Ok(())
    ...
    fn execute_decode(&mut self, token_id: u32, model: &MappedModel) -> Result<Vec<f32>> {
        ...
        unsafe {
            ffi::ggml_backend_tensor_set(self.d_pos, &pos as *const i32 as *const c_void, 0, 4);
        }
        ...
        let status = unsafe { ffi::ggml_backend_graph_compute(self.backend, self.d_graph) };
        if status != 0 {
            bail!("[Z.1 Decode] compute failed status={}", status);
        }
        ...
        if trace_enabled() {
            ...
            unsafe {
                ffi::ggml_backend_tensor_get(
                    self.d_mask,
                    mask_sample.as_mut_ptr() as *mut c_void,
                    0,
                    16,
                );
            }
    ...
    pub fn forward(&mut self, model: &MappedModel, tokens: &[i32], pos: i32) -> Result<Vec<f32>> {
        let hp = &self.hp;
        ...
        let inp_buf = unsafe {
            ffi::ggml_backend_alloc_buffer(self.backend, tok_bytes * 2 + mask_bytes + 384)
        };
        if inp_buf.is_null() {
            bail!("[Z.1 Graph] ggml_backend_alloc_buffer failed");
        }
        let inp_base = unsafe { ffi::ggml_backend_buffer_get_base(inp_buf) } as usize;
        ...
        for layer in 0..hp.n_layer as usize {
            ...
            let k_3d = unsafe {
                ffi::ggml_reshape_3d(ctx, ffi::ggml_cont(ctx, k_read), hd, hp.n_head_kv, kv_len)
            };
            let v_3d = unsafe {
                ffi::ggml_reshape_3d(ctx, ffi::ggml_cont(ctx, v_read), hd, hp.n_head_kv, kv_len)
            };
        ...
        let mut out = vec![0.0f32; n_vocab];
        unsafe {
            ffi::ggml_backend_tensor_get(logits, out.as_mut_ptr() as *mut c_void, 0, n_vocab * 4);
        }
